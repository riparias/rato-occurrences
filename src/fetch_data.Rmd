...
Download requested data as csv and load them  as `raw_occurrences`:

```{r download_data}
raw_occurrences <- 
  httr::GET(url = request) %>% 
  httr::content(na = "",
...
## Remove all Opmerkingen columns

It is possible personal information is entered in the `Opmerkingen` fields, we want to avoid releasing this information by removing them in the file that is written to `data/raw`.

```{r}
raw_occurrences_to_write <-
  dplyr::select(raw_occurrences, -tidyselect::starts_with("Opmerkingen"))
```
...
readr::write_csv(
  x = raw_occurrences_to_write,
  file = file_path,
  na = "",
)

# remove the raw data object without the `Opmerkingen` columns from the
# environment to avoid confusion
rm(list = "raw_occurrences_to_write")
