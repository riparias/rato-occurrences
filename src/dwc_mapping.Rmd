...
```
The `Opmerkingen` fields sometimes contain personal information, thus to avoid publishing them on Github, they are encrypted using a key stored as a GITHUB SECRET. 
```{r decrypt Opmerkingen fields}
decrypt_column <- function(column_to_decrypt, key) {
  purrr::map_chr(column_to_decrypt, ~
                   stringr::str_extract(pattern = "(?<=\\|).+", string = 
                                          safer::decrypt_string(.x, key = key)))
}
...
raw_data <- 
  raw_data %>%
      mutate(
        across(
          tidyselect::starts_with("Opmerkingen"),
          ~ decrypt_column(.x, key = Sys.getenv("opmerkingen_key"))
          )
        )
